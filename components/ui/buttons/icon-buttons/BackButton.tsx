...
const size = 32
function BackArrow({ white = false }: { white?: boolean }) {
    ...
    <Canvas style={{ width: size, height: size }}>
      <FitBox src={{ x: 0, y: 0, width: 40, height: 40 }} dst={{ x: 0, y: 0, width: size, height: size }}>
        <Path
          ...
          color={white ? 'white' : colors[theme].label}
        />
      </FitBox>
    </Canvas>
  )
...
export function BackButtonBlur({ entering = ZoomIn, exiting = FadeOut, tint = undefined }: Props & { tint?: 'dark' | 'light' }) {
  ...
  return (
    <AnimatedBlurView
      ...
      entering={entering}
      exiting={exiting}
      style={{ overflow: 'hidden', borderRadius: 100, padding: 4, backgroundColor: colors[theme].platformMaterial }}
    >
      <GeneralButton onPress={router.back} hitSlop={24} style={{ borderRadius: 100 }}>
        <BackArrow white />
      </GeneralButton>
